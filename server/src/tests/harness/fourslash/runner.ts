import { IO, virtualFileSystemRoot } from "../harness";
import { FourSlashTestType, Marker, Range, FourSlashFile, FourSlashData, fileMetadataNames, MetadataOptionNames } from "./fourSlashTypes";
import { combinePaths, isRootedDiskPath } from "../../../common/pathUtils";
import { contains } from "../../../common/collectionUtils";

export function runFourSlashTest(basePath: string, testType: FourSlashTestType, fileName: string) {
    const content = (IO.readFile(fileName)!);
    runFourSlashTestContent(basePath, testType, fileName, content);
}

export function runFourSlashTestContent(basePath: string, testType: FourSlashTestType, fileName: string, content: string) {
    ...
    const output = transpileModule(content, { reportDiagnostics: true, compilerOptions: { target: ScriptTarget.ES2015 } });
